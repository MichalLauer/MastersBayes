MCMC methods leverage an MH random walk through the parameter space to create samples arbitrarily close to the
...
proposal distribution, the convergence time is prohibitive \parencite[Chap. 12.4]{Gelman2014_BayesianDataAnalysis}.
...
Hamiltonian Monte Carlo (HMC) methods do not rely on MH random walk but with Hamiltonian mechanics. They offer a link
...
First, the HMC algorithm draws initial values for $\phi$ from the jump distribution. Then, parameters $\theta$ and
jumps $\phi$ are simultaneously updated in multiple iterations using $L$ \enquote{leapfrog steps}, where each step
comprises two parts. First, the jumps are updated with the gradient as
...
repeated. As well as the MCMC algorithms, HMC is guaranteed to converge to a specific stationary distribution, the
...
either set manually or adaptively. As \parencite[Chap. 12.4]{Gelman2014_BayesianDataAnalysis} notes that changing the
parameters mid-inference can lead to instability, and the adaptive phase should be run only during the warmup phase.

Even though the method offers higher accuracy in small samples, it is computationally more costly, and MCMC methods
could be better in relatively simple, non-complex models. It is also limited to continuous parameters, and imputing
discrete probability distributions is impossible [^hmc-disc]
