...
the accuracy, and researchers may prefer speed with and quick inference with no guarantees that the model represents the exact posterior
...
is the true posterior, VI tries to find a distribution that is closes to the posterior distribution based on a Kullback–Leibler Divergence (KLD).
It is a measure of how two probability distribution are close to each other. In VI, it measures the distance between the true posterior 
$P(\theta | x)$ and arbitrary distribution function $q$. This measure naturally raises in many statistical methods. One example
...
Applying @eq-kld to IV and reversing the fraction in $\log (\cdot)$[^log-reverse], the equation transforms to
...
The smaller the KLD in @eq-kld-bayes is, the closer is the proposed distribution $q$ to the true posterior $P(\theta|x)$ becomes.
There are many different measures that could be used, however, \parencite{Sjolund2023_TutorialParametricVariational} show that
...
Because the KLD is always non-negative[^nonneg], maximizing $\text{ELBO}(q(\theta))$ is the same as minimizing KLD. This is useful because 
...
\parencite[Chap. 13.7]{Gelman2014_BayesianDataAnalysis} uses $q(x|\phi)$, where $\phi$ denotes such parameters. 
...
Even though the function $g(\theta | \phi)$ might not be exact, VI is already being used in research projects.
