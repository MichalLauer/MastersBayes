...
is the true posterior, VI tries to find a distribution that is close to the posterior distribution measured by a Kullback–Leibler Divergence (KLD).
It is a measure of how two probability distribution are simmilar to each other. In bayesian statistics, it measures the distance between the true posterior $P(\theta | x)$ and arbitrary distribution function $q$. Interestingly, KLD naturally raises in many statistical methods. One example
