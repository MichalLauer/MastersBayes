...
The function $J_t$ in @eq-mha denotes the proposal distribution at iteration $t$. In a simple MA, the ratio of the two conditional
...
In general, \parencite[Chap. 6.8]{Hebak2013_StatistickeMysleniNastroje} suggest that in general, the only condition for the MHA to generate
...
![Simulation of a beta-binomial model using the Metropolis-Hastings algorithm.](./img/sim_ma.png){#fig-sim-mha}

The simulation looks fairly well for the middle model. In smaller or larger true population relative frequency, the simulation fails
to capture the skewness of the true posterior distribution. This can attributed to several factors, such as inappropriate proposal distribution,
insufficient sample size or large autocorrelation.