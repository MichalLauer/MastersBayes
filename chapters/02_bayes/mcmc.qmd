...
from approximate distributions and the correcting those draws to better approximate the target posterior distribution.}*. In statistics, Monte Carlo
simulations are used to sample data from a known population distribution, given some inputs. This allows researchers to compare different methods
...
are simply *\enquote{assessing the properties of a target distribution by generating representative random values.}* \parencite[Chap. 7.4.5]{Kruschke2015_DoingBayesianData}. The analytical equation for a posterior distribution is often unknown, and the use of a simple Monte Carlo 
...
$$
P(\theta^{[t+1]} \in A | \theta^{[0]}, \theta^{[1]}, \dots, \theta^{[t-1]}, \theta^{[t]}) =
...
can move between low-density and high-density regions. If the chain is stationary[^mc-stan], it can converge to a stationary distribution
...
There are several methods that use the MCMC methodology. The most popular methods are the Metropolis–Hastings algorithm (MHA), 
Metropolis algorithm (MA), and the Gibbs sampler (GS), which are later explored in this thesis. Although MCMC sampling is guaranteed to 
...
A simple idea is that after the chain has stabilized, the estimated parameters should not change very much. Plotting the estimated parameter
...
and observing whether all chains converge to the value. Similarly, plotting a histogram of the estimated parameter from multiple chains
...
function so the results autocorrelation analysis and interpretation of ESS should give a similar conclusion. The quality of chains is often heavily
