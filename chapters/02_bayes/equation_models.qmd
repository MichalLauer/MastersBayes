...
with a $\text{Beta}(a, b)$ distribution as its range is bound to $\left< 0, 1\right>$. The likelihood of the data can be expressed by the binomial distribution $\text{Bin}(x|\theta, n)$. Finally, the predictive probability of the data can be rewritten using the law of total probability \parencite[Page 54]{Marek2012_Pravdepodobnost} as

$$
P(x) = \int_{-\infty}^{\infty} P(x|\theta)P(\theta) d\theta.
$$

Combining this information with Equation @eq-bayes-eq gives rise to the beta-binomial model
...
The posterior distribution is analytically tractable and is another beta distribution with parameters $a + y$ and $b + (n - y)$, 
...
The beta-binomial model is also popular because the posterior distribution is analytically tractable. That is because the
...
distribution can be obtained in closed form. In the past, selection of specific functions and distributions that are conjugate has been
...
is proportional to the combination of apriori distribution $P(\theta)$ and likelihood of the data $P(x|\theta)$.

The choice of the apriori distribution depends on the knowledge that is known before the analysis. If, for example, it 
