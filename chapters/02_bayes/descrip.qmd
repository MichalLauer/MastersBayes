To describe bayesian statistics, it is useful to first summarize frequentist statistics. The comparison and
description of both approaches is discussed in \parencite[Chap. 6]{Hebak2013_StatistickeMysleniNastroje}. The frequentist approach
...
all of them can have individual apriori beliefs about the (model) parameter. This means that the probability of an event is considered to be subjective
...
to some set of special distributions which are analyticaly tractable, but do not have to describe the real world in the most precise way.
...
that are connected to bayesian data analysis. In small samples, the posterior degree of belief is heavily affected by the apriori belief that
