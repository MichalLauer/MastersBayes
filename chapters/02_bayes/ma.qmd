...
initial sample that will be used. Since the stationary distribution of markov chain does not depend on the starting value, it can be any
...
where $p$ denotes the density of the posterior distribution for $\theta^*$, defined in @eq-bayes-prop.
If the ratio is greater than 1, it means that the density of the proposed sample $t$ is larger than the density of $t-1$ and the sample is
...
conditioned on $\theta_{t-1}$ is generated and the process repeats.
...
\parencite{MbalawataEtAl2013_AdaptiveMetropolisAlgorithm} notes that the simple MA can be greatly improved with a carefull tuning of the
...
the optimal covariance matrix $\Sigma$ should be multiplied by a coefficient $\lambda = 2.38^2/d$, where $d$ is the dimension of the matrix.
...
model defined in @eq-beta-binomial. Because in this model, the true posterior distribution is known, the accuracy of the simulation can be easily
compared. The line chart represents the true posterior distribution while the histogram the simulated data of iterations with no burn-in period.

![Simulation of a beta-binomial model.](./img/sim_ma.png){#fig-sim-ma}

The simulation generates 10 000 iterations with no burn-in period with proposal description $N(\theta_{t-1}, \sigma = 1)$. For this simulation,
...
binomial distribution for values greater than 1 or smaller than 0. The exact code can be found in appendix \ref{code-ma}.
