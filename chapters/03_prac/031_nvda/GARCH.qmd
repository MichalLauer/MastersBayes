...
model, the volatility sharply drops after just a few time points. The bottom middle model considers a lag of 8
Moreover, the single shock has a large effect on the volatility that follows. This property can also be observed at the
beginning of the year 2025, where two large spikes happened. In the simplest model, the volatility sharply increases
and decreases, preventing the effect from being heavily affected. In contrast, the most complex model does not sharply
decrease. The period between the two volatility jumps is expected to be more volatile because of the past volatility
preserved in time.
...
the GARCH$(5, 1)$ based on two reasons. First, since all models perform relatively similarly and all match the data
well, there is no reason to pick a more complex model when a simple one is sufficient. Secondly, the literature review
suggests that GARCH models of lower orders are usually preferred, supporting the choice of a simple model. The final
model is shown in Figure @fig-nvda-garch-post-final.
