...
This implies that it is expected degrees of freedom, before observing data, is between $3.53$ and $19.03$ with
probability $0.89$. The expected volatility is 10. Because heavier tails are expected, the degrees of freedom 
$\nu$ are probably smaller. The full distribution is shown in Figure @fig-googl-arch-apr-nu.

![GOOGL/ARCH: Apriori distribution for $\nu$](./img/googl/arch/apriori_nu.png){#fig-googl-arch-apr-nu}
...
that 89% credibility interval of $(0.09, 1.16)$ that is quite wide and allows both low and high values. Full
aprior distribution is in Figure @fig-googl-arch-apr-alpha0.

![GOOGL/ARCH: Apriori distribution for $\alpha_0$](./img/googl/arch/apriori_alpha0.png){#fig-googl-arch-apr-alpha0}

Finally, coefficients $\alpha_i$ for $i = 1, \dots, m$ are modelled using $\Beta(2, 6)$. Because stationairy is 
reinforced in the Stan definitio, coefficients greater than zero are not possible and thus the Gamma distribution
not proper. The 89% credibility interval for all coefficients is $(0.06, 0.51)$ because higher values would
imply high memory that is not assumed in this model. The full prior can be seen in Figure @fig-googl-arch-apr-alphai.

![GOOGL/ARCH: Apriori distribution for $\alpha_i$](./img/googl/arch/apriori_alpha_i.png){#fig-googl-arch-apr-alphai}
...
The expected effect decreases with more lags and for $m \geq 4$ it is around 0.08. The 89% credibility intervals
between ARCH$(m = 1)$ and ARCH$(m = 8)$ are different and it is probable that both models put different emphasis
...
model. ARCH$(m = 1)$ can have only $\alpha_1$, model ARCH$(m = 2)$ only $\alpha_1, \alpha_2$ and so on. Finally, a 
...
used. Since the expected log returns are zero, posterior credibility intervals are at each index $t$ are
...
Simple models from $m = 1$ to $m = 4$ are unable to capture the day-to-day volatility and the credibility intervals
