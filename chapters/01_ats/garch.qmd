The variance for ARCH$(p)$  model given in Equation @eq-var-arch is simple and effective, but sometimes not adequate for 
real data. General autoregressive conditional heteroskedasticity (GARCH) model, proposed by Boreslav 
...
One popular extension is the Exponential generalized auto regressive conditional heteroskedasticity (EGARCH) model
\parencite{Nelson1991_ConditionalHeteroskedasticityAsset},
that is able to capture asymmetric shocks. This means that positive and negative jumps in returns can have various
effect on the implied forecasted volatility. Another extensions, GARCH-in-Mean (GARCH-M)
\parencite{EngleEtAl1987_EstimatingTimeVarying}, uses the calculated 
